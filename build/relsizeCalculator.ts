function calcRelsize()
{
    ...
    for (let index = 0; index < percentagesPerPart.length; index++) {
        ...
        let bpSizePer = bpSizesPerArray[index] as HTMLDivElement;
        
        bpSizePer.innerText = ajustedSizePer.toFixed(0);

        let ajustedSize = percentagesTotal[index];

        let bpSize = bpSizesArray[index] as HTMLDivElement;
...
{
    let relsizeTable = <HTMLTableElement><any> document.getElementById("relsizeTable");
    ...
    cell0.appendChild(createSelect({elementID:`select_${numberOfRows}`, options:[
        ...
        let bpName = <HTMLInputElement><any> document.getElementById(`bpName_${numberOfRows}`);

        bpName.value = strArray[0];

        let relsize = <HTMLInputElement><any> document.getElementById(`bpRelsize_${numberOfRows}`);

        relsize.value = strArray[1];

        let bpNumber = <HTMLInputElement><any> document.getElementById(`bpNumber_${numberOfRows}`);
