"use strict";
function createNewAttack(hasPrepRecoveryAndFlags) {
...
function addUpAttacks(numberOfTabObjects, hasPrepRecoveryAndFlags) {
    ...
    for (let rows = 0; rows < tableRows.length; rows++) {
        ...
        if (hasPrepRecoveryAndFlags) {
            pushObject.pushTo.push(tabs, pushObject.tabObject, "[ATTACK_PREPARE_AND_RECOVER:", getInputElementValue(`preperationTime_${rows}`), ":", getInputElementValue(`recoveryTime_${rows}`), "]\n");
