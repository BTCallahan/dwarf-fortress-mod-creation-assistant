...
function checkPlantType() {
    ...
    let show = document.getElementById(plantType.value);
    show.hidden = false;
...
function createGrowth() {
    ...
    growth.appendChild(createParagraph({
        ...
        elementsToAppend: [
            ...
            createParagraph({ innerText: "GROWTH_NAME:", elementsToAppend: [
                    ...
                    createTextInput({ elementID: `GROWTH_NAME_plural_${growthchildren}`, elementClass: "GROWTH_NAME",
                        titleText: "The plural name for this growth. If lenk blank, 'STP' will be used instead", mouseonchange: () => {
                            let growth = document.getElementById(`GROWTH_NAME_plural_${growthchildren}`);
                            if (growth.value === "") {
                                growth.value = "STP";
                            }
                        },
            ...
            createFieldset({ fieldsetId: `GROWTH_HOST_TILE_fs_${growthchildren}`, legendText: "Growth Host Tiles", elementsToAppend: [
                    ...
                    createButton({ innerText: "Create Growth Host Tile", mouseonclick: () => {
                            let a = document.getElementById(`GROWTH_HOST_TILE_${growthchildren}`);
                                ...
                                elementsToAppend: [
                                    createSelect({ elementName: "GROWTH_HOST_TILE", elementClass: "GROWTH_HOST_TILE",
                                        ...
                                        options: [
                                            ...
                                            createSelectOption({ value: "SAPLING", text: "SAPLING" })
                                        ], defualtValue: "TWIGS",
                                    }),
                    ...
                    createButton({ innerText: "Delete Last Growth Host Tile", mouseonclick: function () {
                            let a = document.getElementById(`GROWTH_HOST_TILE_${growthchildren}`);
                            let numberOfChildren = a.children.length;
                            let g = document.getElementById(`GROWTH_HOST_TILE_para_${growthchildren}_${numberOfChildren - 1}`);
                            g.remove();
                        } })
                ] }),
                ...
                elementsToAppend: [
                    ...
                    createButton({
                        ...
                        mouseonclick: function () {
                            let gt = document.getElementById(`GROWTH_PRINT_${growthchildren}`);
...
function readGrowth() {
    ...
    for (let index = 0; index < growth.children.length; index++) {
        let element = growth.children[index];
        ...
        let growthTrunkHeightPercentsArrayValues = Array.from(element.getElementsByClassName("GROWTH_TRUNK_HEIGHT_PERC")).map(value => {
            return value.value;
        });
        ...
        let growthTimingArrayValues = Array.from(element.getElementsByClassName("GROWTH_TIMING")).map(value => {
            return value.value;
        });
        ...
        let growthPrintTable = document.getElementById(`GROWTH_PRINT_${index}`);
        ...
        for (let r = 0; r < tableBodyRows.length; r++) {
            ...
            let cellInputValues = Array.from(currentRow.cells).map(value => {
                let valueChild = value.children[0];
        ...
        let dropOff = document.getElementById(`growthDropOff_${index}`).value;
        if (dropOff !== "NODROP") {
...
function createMaterialTemplate() {
    let materialTemplates = document.getElementById("materialTemplates");
    let numberOfTemplates = materialTemplates.children.length;
    materialTemplates.appendChild(createParagraph({ elementId: `USE_MATERIAL_TEMPLATE_${numberOfTemplates}`, elementsToAppend: [
...
function readMaterialTemplate() {
    ...
    for (let index = 0; index < materialTemplates.children.length; index++) {
        let element = materialTemplates.children[index];
        let templateType = document.getElementById(`templateType_${index}`);
        let matTemplate = document.getElementById(`matTemplate_${index}`);
        ...
        let displayColors = Array.from(element.getElementsByClassName("DISPLAY_COLOR")).map(value => {
            return value.value;
        });
